...
export function getJobStatus(job: Job, year: number): JobStatus {
    ...
    let riskScore = 0.5;
    let color = '#fbbf24';
    if (isHighRisk) { riskScore = 0.9; color = '#ef4444'; }
    else if (isInsulated) { riskScore = 0.1; color = '#22c55e'; }
    ...
    const isProjectedHighRisk = projectedAiCap > 0.7;
    const isProjectedInsulated = effectiveHumanCriticality > 0.65;
    if (isProjectedInsulated) { riskScore = 0.2; color = '#22c55e'; }
    else if (isProjectedHighRisk) { riskScore = 0.9; color = '#ef4444'; }
