    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Live Job Analysis: ${job.title}`} size="lg" layer="top">
            ...
            {isLoading ? (
                ...
                // Compute the true mathematical average of the LIVE task scores generated by Claude.
                // This bypasses Claude's lazy 'overall_ai_impact' hallucination AND avoids the round numbers in the static dataset.
                ...
                const humanDisplay = 100 - aiDisplay;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500 text-xs mt-0.5">Task displacement probability</p>
                                </div>
                            </div>
                            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500 text-xs mt-0.5">Necessity of human judgment</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                                {result.tasks.map((task, i) => {
                                    // Use Claude's live dynamic score for this specific task
                                    const taskAiDisplay = Math.round(task.ai_exposure_score * 100);
                                    const taskHumanDisplay = 100 - taskAiDisplay;
                                        ...
                                        <div key={i} className="p-4 bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.06] rounded-lg transition-colors">
                                            <div className="flex justify-between items-start gap-4">
                                                ...
                                                <div className="flex gap-2 shrink-0">
                                                    <span className={`px-2 py-1 rounded text-[10px] font-semibold uppercase border ${task.ai_exposure_score > RISK_THRESHOLDS.AUTOMATABLE_AI_SCORE ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-white/[0.04] text-gray-500 border-white/[0.08]'}`}>
                                                        AI {taskAiDisplay}%
                                                    </span>
                                                    <span className={`px-2 py-1 rounded text-[10px] font-semibold uppercase border ${taskHumanDisplay > (RISK_THRESHOLDS.HUMAN_CRITICAL_SCORE * 100) ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-white/[0.04] text-gray-500 border-white/[0.08]'}`}>
